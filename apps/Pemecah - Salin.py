    ...
    try:
        with open(txt_file_path, 'r', encoding='utf-8') as txt_file:
            ...
            for index, line in enumerate(lines):
                phone = line.strip()
                if phone:
                    cleaned_phone = re.sub(r'\D', '', phone)  # Hapus karakter selain angka

                    # Filter: Nomor harus memiliki minimal 7 digit dan tidak berupa timestamp
                    if cleaned_phone and len(cleaned_phone) >= 7 and not re.match(r'\d{2}/\d{2}/\d{4} \d{2}\.\d{2}', phone):
                        ...
                        if contact_number % contacts_per_file == 1:
                            ...
                            vcf_file = open(vcf_file_path, 'w', encoding='utf-8')
                            file_number += 1
...
def create_gui():
    ...
    tk.Label(main_frame, text="Masukkan Nama Kontak:").grid(row=3, column=0, sticky="w", pady=5)
    ...
    tk.Label(main_frame, text="Jumlah Kontak per File:").grid(row=4, column=0, sticky="w", pady=5)
    ...
    tk.Label(main_frame, text="Nama File Dasar:").grid(row=5, column=0, sticky="w", pady=5)
    ...
    tk.Label(main_frame, text="Angka Mulai untuk Penamaan:").grid(row=6, column=0, sticky="w", pady=5)
    ...
    tk.Button(main_frame, text="Pilih File", command=select_files).grid(row=7, column=0, columnspan=2, pady=10)
    tk.Button(main_frame, text="Keluar", command=root.quit).grid(row=8, column=0, columnspan=2, pady=10)
